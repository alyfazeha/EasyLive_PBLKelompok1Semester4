...
class _PemilikKosSecurityViewState extends State<PemilikKosSecurityView> {
  ...
  Future<void> _submitChangePassword() async {
    ...
    try {
      ...
      // 1️⃣ Validasi password saat ini — cek ke tabel profiles dulu
      ...
      if (storedPassword != current) {
        _showError('Password saat ini tidak cocok');
        setState(() => isLoading = false);
        return;
      }

      // 2️⃣ Update password di Supabase Auth
      await supabase.auth.updateUser(
        UserAttributes(password: newPass),
      );

      // 3️⃣ Update password di tabel profiles
      await supabase
          .from('profiles')
